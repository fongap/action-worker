...
| 类型 | 变量名 | 用途 | 权限要求 |
|------|--------|------|----------|
| **机密变量** | `GH_SOURCE_VAULT_REPO` | 私密资产仓路径
（存储所有脚本、配置和源代码的私密仓库地址，格式：owner/repo） | - |
| **机密变量** | `GH_SOURCE_VAULT_PAT` | 私密资产仓访问令牌
（用于从私密资产仓拉取脚本和配置） | `repo` 读取权限 |
| **机密变量** | `GH_TARGET_REPO_PAT` | 目标仓库推送令牌
（用于向外部仓库推送构建产物） | `repo` 写入权限 |
